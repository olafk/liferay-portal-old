...
import com.liferay.portal.tools.db.migration.schema.importer.jdbc.ConnectionConfigUtil;
 ...
 */
public class DBMigrationSchemaImport {
	...
	public static void main(String[] args) throws Exception {
		...
		try {
			...
			DBMigrationSchemaImportProcess dbMigrationSchemaImportProcess =
				new DBMigrationSchemaImportProcess(
			...
			dbMigrationSchemaImportProcess.run();

			try (PrintWriter printWriter = new PrintWriter(
					new File(
						commandLine.getOptionValue("path"),
						"db_migration_schema_import_report.txt"))) {
				...
				printWriter.println(
					StringUtil.merge(
						new Object[] {
							...
							dbMigrationSchemaImportProcess.getReleaseInfo(),
							StringPool.NEW_LINE, StringPool.NEW_LINE,
							dbMigrationSchemaImportProcess.getDataSourceInfos()
						},
						StringPool.NEW_LINE));
			}
	...
	private static void _printHelpAndExit(Options options) {
		new HelpFormatter(
			...
			"Liferay Portal Tools Database Migration Schema Importer. This " +
				"tool is a beta feature. It is experimental and not supported.",
			options
		);
