...
public abstract class BaseMessageResourceTestCase {
	...
	public void testGetMessagesPage() throws Exception {
		Page<Message> page = messageResource.getMessagesPage(
		...
		page = messageResource.getMessagesPage(null, null);
	...
	public void testGraphQLGetMessagesPage() throws Exception {
		...
		JSONObject messagesJSONObject = JSONUtil.getValueAsJSONObject(
			invokeGraphQLQuery(graphQLField), "JSONObject/data",
			"JSONObject/messages");

		long totalCount = messagesJSONObject.getLong("totalCount");

		Message message1 = testGraphQLGetMessagesPage_addMessage();
		Message message2 = testGraphQLGetMessagesPage_addMessage();

		messagesJSONObject = JSONUtil.getValueAsJSONObject(
			invokeGraphQLQuery(graphQLField), "JSONObject/data",
			"JSONObject/messages");

		Assert.assertEquals(
			totalCount + 2, messagesJSONObject.getLong("totalCount"));

		assertContains(
			message1,
			Arrays.asList(
				MessageSerDes.toDTOs(messagesJSONObject.getString("items"))));
		assertContains(
			message2,
			Arrays.asList(
				MessageSerDes.toDTOs(messagesJSONObject.getString("items"))));

		// Using the namespace portalLanguage_v1_0

		messagesJSONObject = JSONUtil.getValueAsJSONObject(
			invokeGraphQLQuery(
				new GraphQLField("portalLanguage_v1_0", graphQLField)),
			"JSONObject/data", "JSONObject/portalLanguage_v1_0",
			"JSONObject/messages");
	...
	protected Message testGraphQLGetMessagesPage_addMessage() throws Exception {
		return testGraphQLMessage_addMessage();
	}

	@Test
	public void testPostMessage() throws Exception {
		@SuppressWarnings("PMD.UnusedLocalVariable")
		Message message = testPostMessage_addMessage();

		assertHttpResponseStatusCode(
			204, messageResource.postMessageHttpResponse(null, null));

		assertHttpResponseStatusCode(
			404, messageResource.postMessageHttpResponse(null, null));
	}

	protected Message testPostMessage_addMessage() throws Exception {
		throw new UnsupportedOperationException(
			"This method needs to be implemented");
	}
	...
	public void testPutMessage() throws Exception {
		Message postMessage = testPutMessage_addMessage();

		Message randomMessage = randomMessage();

		Message putMessage = messageResource.putMessage(randomMessage);

		assertEquals(randomMessage, putMessage);
		assertValid(putMessage);

		Message getMessage = messageResource.getMessage();

		assertEquals(randomMessage, getMessage);
		assertValid(getMessage);
	}

	protected Message testPutMessage_addMessage() throws Exception {
		throw new UnsupportedOperationException(
			"This method needs to be implemented");
	}

	@Test
	public void testDeleteMessageByKey() throws Exception {
		@SuppressWarnings("PMD.UnusedLocalVariable")
		Message message = testDeleteMessageByKey_addMessage();

		assertHttpResponseStatusCode(
			204, messageResource.deleteMessageByKeyHttpResponse(null, null));
	}

	protected Message testDeleteMessageByKey_addMessage() throws Exception {
		throw new UnsupportedOperationException(
			"This method needs to be implemented");
	}

	protected Message testGraphQLMessage_addMessage() throws Exception {
		throw new UnsupportedOperationException(
			"This method needs to be implemented");
	}
	...
	protected boolean equals(Message message1, Message message2) {
		...
		for (String additionalAssertFieldName :
			...
			if (Objects.equals("id", additionalAssertFieldName)) {
				if (!Objects.deepEquals(message1.getId(), message2.getId())) {
					return false;
				}
			...
			if (Objects.equals("modifiedDate", additionalAssertFieldName)) {
				if (!Objects.deepEquals(
						message1.getModifiedDate(),
						message2.getModifiedDate())) {
			...
			if (Objects.equals("override", additionalAssertFieldName)) {
				if (!Objects.deepEquals(
						message1.getOverride(), message2.getOverride())) {
	...
	protected String getFilterString(
		...
		if (entityFieldName.equals("createDate")) {
			if (operator.equals("between")) {
				...
				sb.append(_dateFormat.format(message.getCreateDate()));
			}
		...
		if (entityFieldName.equals("modifiedDate")) {
			if (operator.equals("between")) {
				Date date = message.getModifiedDate();
				...
				sb.append(_dateFormat.format(message.getModifiedDate()));
			}
